...
class LayerNorm(nn.Module):
    ...
    LayerNorm normalizes each token.
    """
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            # scaling is important, otherwise the values of each attention entries are too large
            # And this makes the softmax very peaky, which makes the training unstable.
            att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
            att = att.masked_fill(self.bias[:, :, :T, :T] == 0, float("-inf"))
            att = F.softmax(att, dim=-1)
            # randomly prevents some communication between tokens
        ...
        # output projection
        # Apply dropout immediately before the residual connection
...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        # four times the input channels.
        # This number comes from the paper "Attention is all you need" Section 3.3
        self.c_fc = nn.Linear(
...
class Block(nn.Module):
    """Transformer Block: communication (self-attention layer) followed by computation (mlp layer)"""
